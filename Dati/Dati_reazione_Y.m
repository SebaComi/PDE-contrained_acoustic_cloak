...
global u_discreto vertici_mesh
% Source term
data.force = @(x,y,t,param) 0*1 + 0.*x;

% Dirichlet
data.bcDir = @(x,y,t,param) 0*x; % (sin(2*pi*x).*sin(2*pi*y) + 0*x.*y);  
...
% diffusion
data.diffusion = @(x,y,t,param) 1 + 0.*x;
...
% reaction
data.reaction = @(x,y,t,param) 0.*x; %stimatore_dist(x,y,u_discreto,vertici_mesh);
